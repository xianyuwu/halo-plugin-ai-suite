...
public class WritingService {
    ...
    public Flux<AssistEvent> assistStream(String text, String action, String instruction) {
        return Mono.zip(
            ...
            ValidationResult v = validate(text, action, writingCfg);
            if (!v.ok()) {
    ...
    private ValidationResult validate(String text, String action,
        ...
        if (writingCfg != null && !writingCfg.isEnabled()) {
            return ValidationResult.err("AI 写作辅助未启用");
        }
        if (text == null || text.isBlank()) {
            return ValidationResult.err("请先选择要处理的文本");
        }
        int maxInputLength = writingCfg != null && writingCfg.getMaxInputLength() > 0
            ? writingCfg.getMaxInputLength()
            : MAX_INPUT_LENGTH;
        if (text.length() > maxInputLength) {
            return ValidationResult.err("原文超过 " + maxInputLength
