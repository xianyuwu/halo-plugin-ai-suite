...
import run.halo.ai.suite.service.ChatLogger.FeedbackFilter;
import run.halo.ai.suite.service.ChatLogger.LogFilter;
import run.halo.ai.suite.service.ChatLogger.PageResult;
import run.halo.ai.suite.service.ChatLogger.StatsResult;
 ...
 * 后台访客问答日志 + 反馈管理 API.
 *
 * <p>所有路由都在 {@code /apis/console.api.ai-suite.halo.run/v1alpha1/chat-logs/...} 前缀,
 * 由 Halo 自动要求管理员认证.
 */
@Slf4j
@Component
...
public class ConsoleChatLogEndpoint implements CustomEndpoint {
    ...
    /** GET /chat-logs/stats?from=&to= */
    private Mono<ServerResponse> handleStats(ServerRequest request) {
        Instant from = parseInstant(request.queryParam("from").orElse(null));
        Instant to = parseInstant(request.queryParam("to").orElse(null));
        return chatLogger.getStats(from, to)
            .map(this::toJson)
